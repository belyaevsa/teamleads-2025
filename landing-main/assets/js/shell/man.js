...
export function makeMan() {
  var MANPAGES = {
    ...
    agents: 'agents – рецепты агентов от участников: промпты, скиллы, харнессы. cat agents/<имя> – открыть рецепт здесь. Свой промпт можно прислать в чат с пометкой «в рецепты».',
    ...
    team: 'team – тимагочи «Тимлид»: вырасти разработчика и команду в консоли. Анимированный напарник в HUD-полосе реагирует на метрики (доверие, экспертиза, настрой, конфликт) и финансы (деньги, доход, расход, запас). Старт: team new [coder|frontender|teamlead]. Действия только через team <действие>: team 1on1 [имя] (+доверие, -конфликт), team mentor [имя] (+экспертиза, +опыт), team cr (код-ревью), team pair, team delegate, team retro (-конфликт), team hire, team fire <имя>, team ship (копит прогресс релиза), team standup (прожить день). Управление: team (дашборд), team reset, team help. Цель – дорасти до «тимлида» и выкатить 5 релизов; старшие грейды ускоряют ship, но дороже для бюджета. Команда подкидывает ветвящиеся инциденты (реши team a/b/c). team hire – сценарий найма: позови кандидата (team hire 1), задай вопросы (team a/b/c), реши team yes/no. team style – твой архетип лидерства (Пожарный/Садовник/Капитан/Дипломат/Архитектор). team log – хроника решений как файл в твоей ФС (cat team/log.md, tail team/log.md). team share – ссылка-результат для шеринга (Web Share/Telegram/буфер), team result <код> открывает чужой результат. Состояние хранится в браузере и со временем проседает, если игру забросить. Синонимы: tamagotchi, тимагочи, pet.',
    ...
    anon: 'anon <вопрос> – задать вопрос в чат сообщества анонимно. Текст уходит на модерацию админу и публикуется в чате от имени бота @temlead_helper_bot. Автор нигде не сохраняется: ни имени, ни телеграм-id, ни IP – только текст и хеш для защиты от спама. Без аргументов – справка и ссылка на форму /anon/. Синонимы: анонимно, спросить.',
